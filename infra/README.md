...
> **Наслідок для MER-44.** Сховище живе в тій самій базі, яку PowerSync
> реплікує. Публікація `powersync` мусить перелічувати таблиці явно —
> `CREATE PUBLICATION powersync FOR TABLE ...`. Варіант `FOR ALL TABLES`
> затягне в реплікацію власні таблиці PowerSync і зациклить сервіс.
...
**Користувач реплікації — `postgres`.** Так робить офіційне демо PowerSync.
Виділена роль `powersync_role` з `REPLICATION BYPASSRLS` — правильніше, але це
SQL, а SQL у цьому проєкті живе в міграціях: заводити її разом зі схемою в
MER-44.
