...
FROM node:22-alpine
# Та сама база — та сама дірка, тож і те саме лікування, що у web.Dockerfile
# (MER-67): пакети Alpine оновлюємо в межах гілки, прибитої тегом бази. Образ
# не публікується й живе секунди, але рядок коштує один шар і знімає ту саму
# сімку High, тож тримати тут відомо застарілий libssl нема заради чого.
