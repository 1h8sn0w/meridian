 ...
 *
 * Усе тут необов'язкове. Міграція з V1 рядка без жодного з чотирьох полів не
 * створює (`migrateRecipe`), але порожній рецепт як стан можливий: користувач
 * стер усе, що додав, — і рядок лишається порожнім, а не зникає (чому саме
 * так — у `saveRecipe`).
 */
export type Recipe = {
  id: string
