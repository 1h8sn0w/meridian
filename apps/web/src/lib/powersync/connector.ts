...
export async function applyCrudEntry(
  ...
  switch (entry.op) {
    case UpdateType.PUT:
      // `deleted_at: null` перед даними — не косметика, а наслідок виведених id
      // (MER-57). PUT — це вставка, тобто рядок ЖИВИЙ за визначенням; але з
      // виведеним id він може зіткнутися на сервері з власним м'яко видаленим
      // рядком (позначку зняли, а потім поставили наново). Upsert оновлює лише
      // передані колонки, а PowerSync порожніх колонок у PUT не передає — тож
      // без цього рядок лишився б із `deleted_at`, зник би з вибірки sync-правил
      // і позначка тихо скасувалася б сама.
      //
      // Знання про таблиці це не додає: м'яке видалення в цій схемі є в кожній
      // таблиці без винятку (MER-44), а рядок, який щойно створили, видаленим
      // не буває. Значення з самого рядка, якщо воно там є, лишається головним.
      return await table.upsert({ deleted_at: null, ...data, id: entry.id })
  ...
  return {
    ...
    async uploadData(database: CommonPowerSyncDatabase): Promise<void> {
      ...
      try {
        ...
        // частковий унікальний індекс не пустить ДРУГУ вставку того самого
        // рядка з іншого пристрою, бо LWW розсуджує зміни наявного рядка, а не
        // дві незалежні спроби створити його. Ліки не тут: конектор навмисно
        // тонкий і природних ключів таблиць не знає. Той, хто пише рядок, дає
        // йому id, ВИВЕДЕНИЙ із природного ключа (`sync-ids.ts` у ядрі), — тоді
        // обидві вставки стають upsert одного рядка (MER-57).
        console.error(
